    ...
    imgui.Begin("Mining Helper v3.3.5 Beta", showMenu, imgui.WindowFlags.NoDecoration)
        ...
        if targetTime and not active then
            ...
            if rem > 0 then
				...
				local height = 4 -- Сделаем чуть толще для эффекта огня

				-- Настройки «матча»
				local speed = 1.5 -- Секунд на один полет
				local t = (os.clock() % (speed * 2)) / speed -- Цикл от 0 до 2
				local side = t > 1.0 -- false = летит вправо, true = летит влево

				-- Прогресс одного броска (0.0 -> 1.0)
				local p = side and (2.0 - t) or t

				-- Эффект «удара»: резкий старт и плавное замедление к концу (Ease Out)
				local function easeOutQuart(x) return 1 - math.pow(1 - x, 4) end

				local head_p = easeOutQuart(p)
				local tail_p = easeOutQuart(math.max(0, p - 0.25)) -- Хвост тянется за головой

				-- Координаты
				local startX = pL.x + (tail_p * width)
				local endX = pL.x + (head_p * width)

				-- Цвета (ABGR формат)
				local col_head = 0xFF00AAFF -- Голубой «огонь» (голова)
				local col_tail = 0x0000AAFF -- Прозрачный (конец хвоста)

				-- Фон (корт)
				drawL:AddRectFilled(imgui.ImVec2(pL.x, pL.y + 2), imgui.ImVec2(pL.x + width, pL.y + 2 + height), 0x15FFFFFF, 10)

				-- Отрисовка «воланчика» со шлейфом
				if not side then
					-- Летит ВПРАВО: хвост слева (прозрачный), голова справа (яркая)
					drawL:AddRectFilledMultiColor(
						imgui.ImVec2(startX, pL.y + 2), imgui.ImVec2(endX, pL.y + 2 + height),
						col_tail, col_head, col_head, col_tail
					)
				else
					-- Летит ВЛЕВО: голова слева (яркая), хвост справа (прозрачный)
					-- Инвертируем позиции для правильного отображения «вылета»
					local revStartX = pL.x + (1 - head_p) * width
					local revEndX = pL.x + (1 - tail_p) * width
					drawL:AddRectFilledMultiColor(
						imgui.ImVec2(revStartX, pL.y + 2), imgui.ImVec2(revEndX, pL.y + 2 + height),
						col_head, col_tail, col_tail, col_head
					)
				end
                imgui.Dummy(imgui.ImVec2(0, 10))
