    ...
    if imgui_windows.dialog.v then
        ...
        imgui.BeginChild('##child_wrapper 1', imgui.ImVec2(500, 0), false)
            ...
            for k, v in pairs(__imDialogData.videocards) do
                ...
                local earnings = utils.getVideocardEarnings(level or 1)

		local currentBtc = tonumber(v[2]:match("([%d%.]+)%s*BTC"))
		...
		if currentBtc and earnings and earnings > 0 then
        	...
        	farmStopHours = hoursLeft
                ...
                avaibleCrypto.btc = avaibleCrypto.btc + (tonumber(v[2]:match("([%d%.]+)%s*BTC")) or 0)
		summaryAverage.btc = summaryAverage.btc + (earnings or 0)
